...
export default async function handler (
  ...
  if (filters.length > 1 && operators.length !== filters.length - 1) {
    return res
      .status(400)
      .json({
        error: 'Number of operators must be one less than number of filters'
      })
  }
  ...
  for (const filter of filters) {
    ...
    if (!filter.type || !filter.field || !filter.table) {
      return res
        .status(400)
        .json({ error: 'Missing required filter parameters' })
    }
  ...
  for (let i = 0; i < ops.length; i++) {
    if (ops[i] === 'or') {
      combinedList[i] = Array.from(
        new Set([...combinedList[i], ...combinedList[i + 1]])
      )
  ...
  for (let i = 0; i < ops.length; i++) {
    if (ops[i] === 'and') {
      combinedList[i] = combinedList[i].filter(id =>
        combinedList[i + 1].includes(id)
      )
      combinedList.splice(i + 1, 1)
      ops.splice(i, 1)
      i--
