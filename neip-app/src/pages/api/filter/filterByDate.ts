...
const tableMapping: Record<TableName, any> = {
...
export default async function handler (
  ...
  if (!date || !field || !constraint || !table) {
    return res
      .status(400)
      .json({ message: 'Missing required fields: date, field, or constraint' })
  }
  ...
  try {
    ...
    return res.status(200).json((exonerees as { id: number }[]).map(exoneree => exoneree.id));
